...
const Rider = () => {
  ...
  const [item, setItem] = useState([{name:"", type: "", id: "",edit: false}]);
  ...
  const handleAdd = () => {
    item?.map((i) => i?.type === "Fruit" ? setItem([...item,{name: text,type:"Fruit" ,id: Date.now()}]) : setItem([...item,{name: text,type: "Drink", id: Date.now()}]))
    setText("")
  };
  ...
  // /filter fruit
  const fruit = item?.filter((i)=> {
  return ( i?.type === "Fruit" && i?.name)
  })
  const drink = item?.filter((i)=> {
   return (i?.type === "Drink" && i?.name)
  })
    ...
    <div className="w-full h-screen bg-slate-100 flex flex-col items-center relative justify-center">
      <div className="w-11/12 bg-white h-5/6 shadow-2xl flex flex-col items-center justify-between">
        ...
        <div className="w-full h-16 flex items-center justify-between px-48 bg-blue-500">
          <h1 className="text-white font-bold text-xl">Total Fruits: {(item.filter((i) => i?.type === "Fruit" && i?.name)).length}</h1>
          <Modal modal={setShowModal}/>
          <h1 className="text-white font-bold text-xl">Total Drinks: {(item.filter((i) => i?.type === "Drink" && i?.name)).length}</h1>
        </div>
        <div className="flex w-full h-[calc(100%-50%-64px)]">
            <ul className="w-1/2 h-full border-r border-t p-4 overflow-auto">
            {fruit.map((i) => (
             
              <li key={i?.id} className="flex w-full items-center justify-between border-b pb-2 shadow-2xl">
                  {i?.edit === true 
                  ?
                  ( <input type="text"  value={i.name} className="outline outline-blue-500" onChange={(e) => {
                    e.stopPropagation();
                    if (e.target.value !== "") {
                      setItem(
                        item.map((v) =>
            ...
            <ul className="w-1/2 h-[100%-50%] border-r border-t p-4 overflow-auto">
            {drink.map((i) => (
              <li  key={i?.id} className="flex w-full items-center justify-between border-b pb-2 shadow-2xl">
                   {i?.edit === true 
                  ?
