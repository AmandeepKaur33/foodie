...
const Cart = () => {
  ...
  // const cart = cartState?.cartItems?.filter((item)=> item?.CId === loginState?.user?.CId)
  return (
          ...
          <tbody className=" w-full mt-14">
           ...
           (cart?.map((item) => (
              <tr className="mt-20 h-24" key={item?.cartId}>
                ...
                <td className="py-4 px-3 text-gray-500 text-xl">
                 <span onClick={() => setDecrement(item?.cartId,item?.cartQty,item?.CId)} className="cursor-pointer">-</span> {item?.cartQty} <span className="cursor-pointer" onClick={() => setIncrement(item?.cartId,item?.cartQty,item?.qty,item?.CId)}>+</span>
                </td>
                <td className="py-4 px-3 text-gray-500 text-xl">
                  {item?.total_Price}
                </td>
            ...
            }} className='fa-solid fa-trash bg-[#FC6180] px-3 py-1 text-xl rounded-xl text-white '></i>
                </td>
              </tr>
            )))
            :
